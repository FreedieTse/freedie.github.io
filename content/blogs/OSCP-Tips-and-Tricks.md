...
---
# Resources
**Before enrolling in PEN-200**: if you are deciding to get the OSCP, and you don't have a time constrain and wants to try to guarantee your pass on the first attempt, I recommend you to start learning from [Hack The Box Academy](https://academy.hackthebox.com/) - [Penetration Tester Path](https://academy.hackthebox.com/path/preview/penetration-tester) for the HTB Certified Penetration Testing Specialist (CPTS) certificate. A lot of people consider the materials taught in CPTS better than the material taught in OSCP, yet it is cheaper. I believe completing the path could range from 3 to 6 months, but you may also skip modules that are irrelevant to the OSCP exam if you like. 
...
*Note: AD Chains 1-6 from Hacker Blueprint uses VirtualBox*
...
- If you got into a database service like `mysql` and can't crack the password for the user; google default password hash or hash generator for web service and update the field to "reset" password
### Exploiting
...
- If you are dealing with python exploits: might need to **specify the python version** to run the exploit properly. e.g.: `python2 [exploit.py]` or `python2.7 [exploit.py]` 
...
- Sometimes `echo [base64 reverse shell] | base64 -d | bash` could work, but I typically get away with `bash -c 'bash -i'` reverseshell in OSCP labs
- **Try the `busybox` reverse shell**: sometimes the lab machine doesn't have `nc`
...
- When dealing with **Windows** machines: use `rlwrap nc -lvnp [port]` as listnere for a more stabilized shell. In addition, use the `PowerShell #3 (Base64)` as payload.
...
- **Bypass simple file extension checks** by intercept and modify with `Burp Suite`
...
- When enumerating a popular/well known web service, we can enumerate with their corresponding wordlists from seclists. e.g. we can search apache wordlists by 
  ```
...
1. Look in web service config files for passwords to access database (typically `mysql`) or spraying that password against users from `/etc/passwd`: such as `wp-config.php` for `wordpress`. If you don't know the config file, google `[web service] default config location` or something similar
...
3. Enumerating strange file in `/opt`, `/var`, the mails if there is a mail service, `/home/[user]` of other users. Also, the `.bash_history`, `.bashrc`, etc. of users.
...
- `whoami /all`: you should know how to abuse tokens such as `SeImpersonate`, etc.
...
- Check for ports hosted internally with `netstat -ano`
...
- Search for credentials: in `PowerShell`/`CMD` history, etc.
  *Note: AD Chains 7 from Hacker Blueprint teaches a very detailed methdology for credential hunting* 
...
- **Practice `netexec`!** The absolute most useful tool in AD. It can basically do everything
	- Learn to make a user list with the output of `netexec smb` (try `--rid-brute`)
	...
	- Use `netexec` to confirm `winrm` or `rdp` privileges (shows golden `(Pwn3d!)`)
...
- Enumerate the `SYSVOL` for `cpassword` in `groups.xml`; decrypt with `gpp-decrypt`
